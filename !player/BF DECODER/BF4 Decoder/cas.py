...
class Entry:
    ...
    def __init__(self,f):
        entryType=f.read(1)
        if entryType=="\x82": #ordinary entry
            entrySize=read128(f)
            entryOffset=f.tell()
...
def addField(f,entry):
    ...
    elif dataType=="\x02": vars(entry)[fieldName]=f.read(read128(f))
...
def unXor(path):
    ...
    elif magic == 0x00D1CE03: #the file has a signature, but an empty key; it's not encrypted
